    ...
    dbc.Container(
        [
            dbc.Row([
                ...
                dbc.Col([
                    html.Div([dcc.Dropdown(id='data-set', multi=False, value='dataset',options=[{'label':'Données', 'value':'dataset'},{'label':'test','value':'essai'}])
                    ], className='row', style={'width':'50%'}),
                    #dcc.store met le data frame dans la session de l'utilisateur
                    dcc.Store(id='store-data',data=[],storage_type='session') #storage des données pendant toute la session
                ],width=2),
                #dcc.store met le data frame dans la session de l'utilisateur
                #dcc.Store(id='store-data',data=[],storage_type='session') #storage des données pendant toute la session
        ]),
...
#ou sauver en tant que string comme dans JSON
#return df.to_json(orient='split')
